...
Para sincronizar a reprodução, a extensão troca pela sala do Owlbear Rodeo informações técnicas temporárias, como identificadores de conexão, estados de carregamento e reprodução, medições de tempo, erros de mídia e informações do navegador. O nome exibido pelo participante é obtido da própria sala. Essas informações são utilizadas durante a sessão para operação e diagnóstico pelo GM.
