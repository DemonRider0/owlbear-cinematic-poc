...
describe("music state anchors", () => {
  it("accepts but strips legacy shared volume fields from new states", () => {
    ...
    expect(isMusicState(legacy)).toBe(true);
    expect(stripLegacyVolumeFields(legacy)).not.toHaveProperty("musicVolume");
    expect(stripLegacyVolumeFields(legacy)).not.toHaveProperty("effectsVolume");
    ...
    expect(next).not.toHaveProperty("musicVolume");
    expect(next).not.toHaveProperty("effectsVolume");
  });
